...
public class VoxyClientInstance extends VoxyInstance {
    ...
    protected SectionStorage createStorage(WorldIdentifier identifier) {
        ...
        ctx.setProperty(ConfigBuildCtx.BASE_SAVE_PATH, this.basePath.toString()); // comment three for unified folder lod
        ctx.setProperty(ConfigBuildCtx.WORLD_IDENTIFIER, identifier.getWorldId()); // and uncomment the five ones above
        ctx.setProperty(ConfigBuildCtx.PLAYER_UUID, Minecraft.getInstance().getUser().getProfileId().toString().replace(':','-'));
